...
class Request {
  ...
  public apiRequest(url: string, contentType: string, method: string, args: { [x: string]: any }, responseType: string) {
    let options = {};
      ...
      requestPromise
        ...
        .then((res) => {
          ...
          if (responseType.toUpperCase() == 'JSON') {
            return resolve({ statusCode: StatusCodes.OK, data: JSON.parse(res) });
          } else if (responseType.toUpperCase() == 'XML') {
            // console.log('resp', res);
            let respJson = parser.xml2json(res, { compact: true, ignoreComment: true, space: 4 }) as any;
